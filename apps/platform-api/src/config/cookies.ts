...
export function getCookieOptions(
  ...
  // In production, use the appropriate domain for the deployment
  const cookieDomain = isProduction
    ? (process.env.COOKIE_DOMAIN || '.xala.no')
    : undefined;
...
export function validateCookieConfig(): { valid: boolean; errors: string[] } {
  ...
  if (isProduction) {
    ...
    if (!process.env.COOKIE_DOMAIN) {
      errors.push('COOKIE_DOMAIN should be set in production');
    }
  }
...
export function getClearCookieOptions(
  ...
  return {
    path: '/',
    domain: isProduction ? (process.env.COOKIE_DOMAIN || '.xala.no') : undefined,
  };
}
