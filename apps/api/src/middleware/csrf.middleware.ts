...
export async function csrfMiddleware(
  ...
  if (!csrfCookie || !csrfHeader || csrfCookie !== csrfHeader) {
    ...
    reply.code(403);
    return reply.send(createErrorResponse(request, 'CSRF_TOKEN_MISMATCH', 'csrf.token_mismatch'));
  }

  // 2. Check Origin header (must be from *.digilist.no or localhost in dev)
  ...
  if (origin) {
    ...
    if (!allowedOrigins.includes(origin)) {
      ...
      reply.code(403);
      return reply.send(createErrorResponse(request, 'INVALID_ORIGIN', 'csrf.invalid_origin'));
    }
  }
  ...
  if (!origin) {
    ...
    if (referer) {
      ...
      if (!allowedOrigins.includes(refererOrigin)) {
        ...
        reply.code(403);
        return reply.send(createErrorResponse(request, 'INVALID_REFERER', 'csrf.invalid_referer'));
      }
    }
  }
