/**
 * Generate 40 Rental Objects with Complete Metadata
 * Uses ONLY rental object terminology (no "listing")
 */

export function generate40RentalObjects(params: {
  TENANT_SKIEN: string;
  ORG_SKIEN_HALL: string;
  RENTAL_HALL_A_ID: string;
}) {
  ...
  function generateSlug(name: string, index: number) {
    return name
      .toLowerCase()
      .replace(/æ/g, 'ae')
      .replace(/ø/g, 'o')
