...
describe('useListingFilters', () => {
  ...
  it('should initialize with custom filters', () => {
    const initialFilters: Partial<ListingQueryFilters> = {
      type: 'SPACE',
      status: 'published',
      search: 'test',
      page: 2,
    ...
    const { result } = renderHook(() => useListingFilters(initialFilters));
  ...
  it('should initialize with custom view mode', () => {
    const { result } = renderHook(() => useListingFilters(undefined, 'grid'));
  ...
  it('should update view mode', () => {
    const { result } = renderHook(() => useListingFilters(undefined, 'table'));
