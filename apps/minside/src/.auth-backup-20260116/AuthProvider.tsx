  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      try {
        ...
        // ---------------------------------------------------------------------------
        // CRITICAL: We must clear user state even if localStorage has cached data.
        // The HTTP-only session cookie is the ONLY source of authentication truth.
        // If the cookie is invalid/expired, the user is NOT authenticated, regardless
        // of what's in localStorage (which could be stale or tampered with).
        //
        // Previous vulnerability: If session cookie expired but localStorage had cached
        // user data, the app would remain "authenticated" without a valid session.
        // This would allow:
        //   ❌ Access to protected routes with expired/invalid session
        //   ❌ API calls that would fail at backend but UI shows "logged in"
        //   ❌ Bypass of authentication after session expiry
        //
        // The fix: Always clear user state when session validation fails, then
        // check browser storage only to clean it up (not to restore authentication).
        
        // Clear user state FIRST (establish unauthenticated state)
        setUser(null);
        
        // Clear any stale cached data from localStorage (cleanup only)
        localStorage.removeItem('minside_user');
        localStorage.removeItem('backoffice_mock_user'); // Clean up any old mock data
