 ...
 * Displays past report generations with filters and download capability
 * Follows patterns from audit.tsx and bookings.tsx
 */
...
const STATUS_TABS = [
  ...
  { id: 'processing', label: 'Behandler', icon: '⏳', color: 'warning' },
  { id: 'pending', label: 'Venter', icon: '⏱', color: 'info' },
  { id: 'failed', label: 'Feilet', icon: '✕', color: 'danger' },
] as const;
...
function getStatusColor(status: ReportJobStatus): 'success' | 'info' | 'warning' | 'danger' {
  switch (status) {
    case 'completed':
      return 'success';
    case 'processing':
...
function getStatusLabel(status: ReportJobStatus): string {
  switch (status) {
    case 'completed':
      return 'Fullført';
    case 'processing':
            ...
            <Table.Body>
              {reports.map((report: ReportHistoryItem) => (
                ...
                >
                  ...
                  <Table.Cell>
                    <Badge color={getStatusColor(report.status)} data-size="sm">
