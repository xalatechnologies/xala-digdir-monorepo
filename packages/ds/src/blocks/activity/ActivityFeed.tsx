 ...
 */
import { Card, Paragraph, Badge } from '@digdir/designsystemet-react';
...
export function ActivityItem({ activity, onClick }: ActivityItemProps) {
  return (
    <div onClick={() => onClick?.(activity.id)} style={{ display: 'flex', gap: 'var(--ds-spacing-3)', padding: 'var(--ds-spacing-3)', cursor: onClick ? 'pointer' : 'default', borderBottom: '1px solid var(--ds-color-neutral-border-subtle)' }}>
      ...
      <div style={{ flex: 1 }}>
        <Paragraph data-size="sm" style={{ margin: 0/*, fontWeight: '500'*/ }}>{activity.title}</Paragraph>
      ...
      {activity.status && <Badge data-size="sm" data-color={activity.status === 'completed' ? 'success' : activity.status === 'cancelled' ? 'danger' : 'warning'}>{activity.status}</Badge>}
    </div>
  );
}
