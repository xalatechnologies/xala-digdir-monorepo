...
export interface RentalObjectCardProps {
  ...
  type: string;
  /** Rental object type from schema: 'SPACE' | 'RESOURCE' | 'EVENT' | 'SERVICE' | 'VEHICLE' | 'OTHER' */
  ...
  /** Number of reviews */
  reviewCount?: number;
  /** Whether the rental object is available */
  available?: boolean;
...
export function RentalObjectCard({
  ...
  showRentalObjectType: _showRentalObjectType = true,
  maxAmenities = 3,
      ...
      <div style={{ padding: 'var(--ds-spacing-6)', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        {showFacilities && facilities.length > 0 && (
          <div style={{
            ...
            {facilities.slice(0, maxAmenities).map((amenity) => (
