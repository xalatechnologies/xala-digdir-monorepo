...
export interface RentalObjectListItemProps {
  /** Unique identifier */
  id: string;
  /** Rental object name/title */
  name: string;
  /** Category/type label */
  type: string;
  /** Rental object type from schema */
  rentalObjectType?: 'SPACE' | 'RESOURCE' | 'EVENT' | 'SERVICE' | 'VEHICLE' | 'OTHER';
  /** Location text */
  location: string;
  /** Description text */
  description: string;
  ...
  /** Whether this rental object is favorited */
  isFavorited?: boolean;
  /** Custom class name */
  className?: string;
...
// Rental object type labels in Norwegian
const rentalObjectTypeLabels: Record<string, string> = {
  SPACE: 'Lokale',
  RESOURCE: 'Ressurs',
  EVENT: 'Arrangement',
  SERVICE: 'Tjeneste',
...
// Rental object type colors for badges
const rentalObjectTypeColors: Record<string, string> = {
  SPACE: 'accent',
  RESOURCE: 'info',
  EVENT: 'success',
  SERVICE: 'warning',
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {showTypeBadge && (
          <div style={{ position: 'absolute', top: 'var(--ds-spacing-3)', left: 'var(--ds-spacing-3)' }}>
            <Tag data-size="sm" data-color={rentalObjectTypeColors[rentalObjectType || 'OTHER'] || 'neutral'} style={{ paddingInline: 'var(--ds-spacing-2)' }}>
              {type}
            </Tag>
          </div>
        )}
      ...
      <div style={{
        ...
        {/* Header with title and rental object type */}
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 'var(--ds-spacing-3)', marginBottom: 'var(--ds-spacing-2)' }}>
          ...
          {showRentalObjectType && rentalObjectType && (
            <Tag data-size="sm" data-color={rentalObjectTypeColors[rentalObjectType] || 'neutral'} style={{ paddingInline: 'var(--ds-spacing-2)', flexShrink: 0 }}>
              {rentalObjectTypeLabels[rentalObjectType] || rentalObjectType}
            </Tag>
          )}
        </div>
