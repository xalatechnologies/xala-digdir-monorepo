import { test as base, expect, Page, BrowserContext, ConsoleMessage as PWConsoleMessage } from '@playwright/test';
import { config } from '../../suites/backoffice/config/backoffice.config';
...
export interface Evidence {
  ...
  consoleLogs: LocalConsoleMessage[];
  apiCalls: ApiCall[];
  pageErrors: string[];
}
...
export class EvidenceCollector {
  ...
  getConsoleErrors(): LocalConsoleMessage[] {
    return this.consoleLogs.filter(
      (log) =>
        log.type === 'error' &&
        !config.allowlistedWarnings.some((w: string) => log.text.includes(w))
    );
  }
...
export { expect }; // Keep this one for now, but ensure no others
