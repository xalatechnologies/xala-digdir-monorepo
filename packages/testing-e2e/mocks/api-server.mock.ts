...
/**
 * Setup function for Playwright tests
 * Pass the Playwright 'test' object to wire up lifecycle hooks
 */
export function setupMockApi(test: any) {
  test.beforeAll(() => mockApiServer.listen({ onUnhandledRequest: 'bypass' }));
  test.afterEach(() => mockApiServer.resetHandlers());
  test.afterAll(() => mockApiServer.close());
}
